...
/**
 * PENDING Outbox 하나를 Kafka topic으로 publish하고 PUBLISHED 처리한다.
...
export async function publishNextPendingOutbox(): Promise<boolean> {
  ...
  try {
    await sendMessage(topicForEventType(outbox.eventType), outbox.jobId, {
      jobId: outbox.jobId,
    ...
    console.error(`[blockchain-worker] outbox publish failed id=${outbox.id} jobId=${outbox.jobId}: ${message}`);
    return false;
  }
}
